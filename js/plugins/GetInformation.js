...
if($gameSwitches.value(infoDisableSwitchId)){//スイッチx番がONのときにポップアップ非表示.変更
return;
}
        var text1 = null;
        if (value === 0) { return }
        ...
        texts = texts.compact();
        if(!$gameParty.inBattle()){$gameSystem.pushInfoLog(text1)};
        //$gameSystem.pushInfoLog(text1);//変更。戦闘中はログ収集中止
        ...
        if (infoPattern === 'GrowUp') {
            arg.x = 0 + infoSupX + valueGetInfoPointX;//Graphics.boxWidth * -1 + infoSupX;
