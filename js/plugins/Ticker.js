...
(function(){
  ...
  TickerManager.show = function(text){
    ...
    $gameSystem.pushInfoLog(text);//追加。インフォログにティッカー内容を直接入れる。
  ...
  TickerManager.hide = function(){
    ...
    for(var i = 0; i < this.tickers.length; i++){
      this.tickers[i].y = this.tickers[i].lineHeight() * i + Y_POSITION+$gameVariables.value(185);//変更
    }
  };
  ...
  Window_Ticker.prototype.initialize = function(index, text) {
    Window_Base.prototype.initialize.call(this, 0, this.lineHeight() * index + Y_POSITION+$gameVariables.value(185), Graphics.width, this.lineHeight());
    this.text = text;
    this.textX = 100;
  ...
  Window_Ticker.prototype.update = function() {
    ...
    if(this.isShowing){
      ...
      if(this.frameCount == $gameVariables.value(71)){//変更
        this.isDisplaying = false;
        this.isHiding = true;
        this.frameCount = 0;
      }
