...
Game_Battler.prototype.onAllActionsEnd = function() {
  ...
  if (this._actionMastery && $gameParty.inBattle()) {
    ...
    while (this._actionMastery.length > 0) {
      ...
      this.gainSkillMasteryUses(skillId, $gameVariables.value(203));//変更。1から変数に
    ...
    if (levelup && Yanfly.Param.SMLAnimation > 0) {
      ...
      this.startAnimation(Yanfly.Param.SMLAnimation, mirror);//変更。バトルメッセージ追加
      BattleManager._logWindow.push(`addText`, `${this.name()}の\\C[10]<${$dataSkills[skillId].name}>\\C[0]がランクアップ！\\C[2]（${this.skillMasteryLevel(skillId)-1}→${this.skillMasteryLevel(skillId)}）\\C[0]`);
      $gameSwitches.setValue(374,true);//変更追加
...
Window_Base.prototype.drawSkillMasteryLevel = function(skill, x, y, width) {
  ...
  var text2 = '(' + this._actor.skillMasteryUses(skill) + ')';//変更追加
  ...
  this.drawText(text2, x + 2, y + offsetY + 16, Window_Base._iconWidth, align);//変更追加
  this.resetFontSettings();
};
