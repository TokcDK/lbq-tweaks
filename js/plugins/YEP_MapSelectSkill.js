...
Window_MapSelectSkill.prototype.setup = function(varId, actorId, stypeId) {
    ...
    this.activate();
    this.open();
    this.select(valueLastChoiceMapSelectSkill);//変更。0から。連続して選択する時に初期カーソルを記憶させるため
};

Window_MapSelectSkill.prototype.includes = function(item) {
    if (!this._stypeId) return item;
    if (item.hideIfLearnedSkill) {//変更。スキルハイド状態のスキルを非表示
      for (var i = 0; i < item.hideIfLearnedSkill.length; ++i) {
...
Scene_Map.prototype.onMapSelectSkillOk = function() {
    ...
    if (!skill) {
      ...
      valueLastChoiceMapSelectSkill = 0;//変更。追加。連続して選択する時に初期カーソルを記憶させるため
    } else {
      $gameVariables.setValue(varId,skill.id);
      valueLastChoiceMapSelectSkill = this._mapSelectSkillWindow._index;//変更。追加。連続して選択する時に初期カーソルを記憶させるため
    }
