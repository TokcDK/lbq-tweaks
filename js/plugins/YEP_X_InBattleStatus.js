...
Window_InBattleStatus.prototype.refresh = function() {
  ...
  if (h >= this.lineHeight() * 6) {
    ...
    this.drawText2(0, x, y, w, this.lineHeight());//変更。追加。通常攻撃種別表示
  } else {
    w = Math.floor(w / 2);
    x2 = x;
    for (var i = 2; i < 8; ++i) {
...
Window_InBattleStatus.prototype.drawText2 = function(paramId, dx, dy, dw, dh) {//変更。追加。通常攻撃種別表示
  ...
  this.drawText($dataSkills[this._battler.attackSkillId()].name, dx, dy, dw);
  var value = this._battler.param(paramId);
  this.changeTextColor(this.paramchangeTextColor(level));
  this.drawText(' Rank:' + this._battler.skillMasteryLevel(this._battler.attackSkillId()) + '/' + Number($dataSkills[this._battler.attackSkillId()].meta['Max Mastery Level']) + ' Uses:' + this._battler.skillMasteryUses(this._battler.attackSkillId()) + '/' + this._battler.skillMasteryUsageMax(this._battler.attackSkillId()), dx, dy, dw, 'right');
...
Window_InBattleStateList.prototype.setBattler = function(battler) {
  ...
  SceneManager._scene._inBattleStatusWindow.show();//変更。4行追加
  SceneManager._scene._inBattleStateList.show();
  SceneManager._scene._helpWindow.show();
  WindowManager.hideAll();
  valueStatusSet1 = 0;
  if(this._battler.isActor()){//変更。追加
