 ...
 *
 * @param Window Sort
 * @desc イベント一覧の並び順を設定します。（id:イベントID dist:主人公からの距離 x:X座標 y:Y座標）
 * @default id
 *
 * @param Show Range
...
(function() {
        ...
        this._choiceEvents = $gameMap.events().filter(function(event) {
            ...
            if(hzChoice == null) {
                ...
                if(variable == 'A' || variable == 'B' || variable == 'C' || variable == 'D') {
                    ...
                    //変更console.log($gameSelfSwitches.value(key));
    ...
    Window_EventList.prototype.initialize = function(x, y, width, height) {
        this._listevents = [];
        Window_Selectable.prototype.initialize.call(this, x, y+70, width, height);//変更yに+70
        this.deactivate();
        this.hide();
    };
    ...
    Window_EventList.prototype.drawItem = function(index) {
        ...
        this.resetTextColor();
        this.drawText(event.name, rect.x, rect.y, rect.width);
    };
