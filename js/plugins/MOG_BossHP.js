...
Game_Enemy.prototype.checkBossHPnoteTag = function() {
	...
	for (var i = 0; i < this.notetags().length; i++) {
		...
		//if ($gameSwitches.value(211)) {this._bosshp_meter = true};
...
Sprite_BossHP.prototype.createSprites = function() {
	this.x = this._hp_data[9]-150;//変更-150
...
Sprite_BossHP.prototype.refreshStates = function() {
	...
	var icons = this._battler.allIcons().slice(0,w);
        var arr1 = [];
        for (i = 0; i < this._battler.allIcons().length; i++){
          if(this._battler.allIcons()[i] == 31 || this._battler.allIcons()[i] == 247 || this._battler.allIcons()[i] == 248 || this._battler.allIcons()[i] == 249 || this._battler.allIcons()[i] == 250 || this._battler.allIcons()[i] == 457){}else{
            arr1.push(this._battler.allIcons()[i]);
          };
        };
        var icons = arr1;
...
Sprite_BossHP.prototype.refreshFace = function() {
    ...
    this._face.bitmap = ImageManager.loadBossHp("Face");//変更
};
