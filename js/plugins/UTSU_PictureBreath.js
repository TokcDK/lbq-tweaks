 ...
 *   例: UTSU.PictureBreath.on([71,72], 150);
 *
...
((global) => {
  ...
  Sprite_Picture.prototype._breathUpdate = function () {
    ...
    if (picture._breathActive) {
      ...
      this.scale.y -= freq * valueTachieBless[4] + valueTachieBless[4]; //0.015 + 0.015
      this.y -= Math.ceil((this.height * (1.0 - this.scale.y)) / valueTachieBless[1]);
      this.scale.x += freq * valueTachieBless[3] + valueTachieBless[3]; //0.005 + 0.005
      this.x += Math.ceil((this.width * (1.0 - this.scale.x)) / valueTachieBless[0]);
    }
  };
})(window);
