...
// ** Game Temp
//=============================================================================
ImageManager.loadMenusFaces7 = function(filename) {
    return this.loadBitmap('img/battleresult/', filename, 0, true);
};	
//==============================
// ** Initialize
//==============================
...
BattleResult.prototype.loadImages = function() {
	...
	this._vict_img = ImageManager.loadSystem("Result_E" + $gameVariables.value(70));
};
...
BattleResult.prototype.createGold = function() {
	...
	for (var i = 0; i < 11; i++) {
	   this._goldNumber[i] = new Sprite(this._number_img2);//変更。img1から2へ。不具合？
...
BattleResult.prototype.playVoice = function() {
	if (Imported.MOG_BattleCry) {SoundManager.selectVoice(this._actor._v_levelup)};
      $gameParty.members()[this._actor.index()].startAnimation(300, false, 0);
};
...
BattleResult.prototype.createActorName = function() {
	...
	this._name.bitmap.drawText(this._actor._name,0,0,200,40);
        this._name.fontSize = 35;//変更。項目ごと追加
...
BattleResult.prototype.createActorSprite = function() {
	var img = ImageManager.loadMenusFaces7("ActorR" + this._actor._actorId +"_"+ $gameVariables.value(380 + this._actor._actorId)[66]);//変更
...
BattleResult.prototype.createParameters = function() {
	...
	this._par.opacity = 0;
        this._par.bitmap.textColor = '#8888ff' ;//fontColor;
        this._par.bitmap.fontSize = 35;//変更。項目ごと追加
	this._par.bitmap.drawText(this._actopar[0],x,y,150,40);
	this._par.bitmap.drawText(this._actopar[1],x,y + 40 * 1,150,40);
	this._par.bitmap.drawText(this._actopar[2],x,y + 40 * 2,150,40);
	this._par.bitmap.drawText(this._actopar[3],x,y + 40 * 3,150,40);
	this._par.bitmap.drawText(this._actopar[4],x,y + 40 * 4,150,40);	
	this._par.bitmap.drawText(this._actopar[5],x,y + 40 * 5,150,40);
	this._par.bitmap.drawText(this._actopar[6],x,y + 40 * 6,150,40);
	this._par.bitmap.drawText(this._actopar[7],x,y + 40 * 7,150,40);
	this._par.bitmap.drawText(this._actopar[8],x,y + 40 * 8,150,40);
	var x = Moghunter.bresult_par2_x;
	var y = Moghunter.bresult_par2_y;
        this._par.bitmap.textColor = '#ffea00' ;//fontColor;	
	this._par.bitmap.drawText(this._actor._level,x,y,150,40);
	this._par.bitmap.drawText(this._actor.mhp,x,y + 40 * 1,150,40);
	this._par.bitmap.drawText(this._actor.mmp,x,y + 40 * 2,150,40);
	this._par.bitmap.drawText(this._actor.atk,x,y + 40 * 3,150,40);
	this._par.bitmap.drawText(this._actor.def,x,y + 40 * 4,150,40);	
	this._par.bitmap.drawText(this._actor.mat,x,y + 40 * 5,150,40);
	this._par.bitmap.drawText(this._actor.mdf,x,y + 40 * 6,150,40);
	this._par.bitmap.drawText(this._actor.agi,x,y + 40 * 7,150,40);
	this._par.bitmap.drawText(this._actor.luk,x,y + 40 * 8,150,40);	
...
BattleResult.prototype.refreshNewSkill = function() {
	...
	//this._skill.fontSize = 35;//変更+2。適用できなかった
...
BattleResult.prototype.updateLevelPosition = function() {
	if (this._phaseAnime[0] === 0) {
		this._phaseAnime[0] = 1;
		this._spriteActor.y = -732;;//-this._spriteActor.height + Moghunter.bresult_actor_y+500;//変更。リザルト立ち絵の縦長さ-768
		this._par.y = this._spriteActor.width + Moghunter.bresult_actor_y;
	};
	this._spriteActor.x = Graphics.boxWidth - this._spriteActor.width + Moghunter.bresult_actor_x;
	this._spriteActor.y = this.sprite_move_to(this._spriteActor.y,Moghunter.bresult_actor_y,20+10);//変更
...
BattleResult.prototype.sprite_move_to = function(value,real_value,speed) {
	if (value == real_value) {return value};
	var dnspeed = 0.5 + (Math.abs(value - real_value) / speed);//変更
