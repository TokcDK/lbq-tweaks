...
Window_ShopBuy.prototype.maxCols = function() { //変更。Sellからの追加処理
    return valueShopMaxCols; //変更.amygamesetteiで指定
};
...
Window_ShopBuy.prototype.drawBuyItem = function(item, rect) {
    this.drawItemName(item, rect.x, rect.y, rect.width - 100); //変更rect.widthに - 100を追加
};
...
Window_ShopStatus.prototype.updateParamSwitch = function() {
    ...
    if (this.isTouched(-1) || this.getInput('left')) {
      ...
      this.adjustRight();
      this.refresh();
    } else if (Input.isRepeated('menu') || this.isTouched(0)) {//変更tabをmenu
      SoundManager.playCursor();
      this.adjustMode();
      this.refresh();
    }
