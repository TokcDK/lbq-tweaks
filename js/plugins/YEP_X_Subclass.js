...
Game_Actor.prototype.setSubclass = function(classId) {
    ...
    if(classId >= 51 && classId <= 80){//変更追加。サブジョブ変更時にジョブスキル装着
      var arr1 = this.battleSkillsRaw();
      for (var i = 0; i <= arr1.length-1; i++) {
        if(arr1[i] == 0){
          if(this.getEquipSkillTierCount(3) < this.getEquipSkillTierMax(3)){
            if($dataClasses[classId].learnings[0].skillId != 0){
              if (!this.battleSkillsRaw().contains($dataClasses[classId].learnings[0].skillId)) {
                this.equipSkill($dataClasses[classId].learnings[0].skillId, i);
                break;
              };
            };
          };
...
Game_Actor.prototype.changeSubclass = function(classId) {
    ...
    this.startAnimation(295, true, 0);//変更。アニメ追加
};
