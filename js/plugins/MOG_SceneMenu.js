 ...
 * @desc Posição X-Axis do número de LV maximo.
 * @default 95
 *
 * @param LV Number Y-Axis
 * @desc Posição Y-Axis do número de LV maximo.
 * @default 33
 *
...
ImageManager.loadMenusFaces1 = function(filename) {
    return this.loadBitmap('img/menusequipfaces/', filename, 0, true);
};	
...
ImageManager.loadMenusFaces3 = function(filename) {
    return this.loadBitmap('img/menusfaces/', filename, 0, true);
};			
...
Scene_Menu.prototype.create = function() {
  ...
  valueMenuStandval3 = 1280;//変更
  valueMenuStandval1 = 0;
  if($gameVariables.value(252) == 5){
    valueMenuStandval2 = $gameParty.battleMembers().length-1;
  } else {
    valueMenuStandval2 = $gameVariables.value(252);
  };
  if($gameVariables.value(252) == 4){
    valueMenuStandval2 -= 1;
    valueMenuStandval1 = 0;
  };
  if($gameVariables.value(252) == 3){
    valueMenuStandval2 -= 1;
    valueMenuStandval1 = 50;
  };
  if($gameVariables.value(252) == 2){
    valueMenuStandval2 -= 1;
    valueMenuStandval1 = 100;
  };
  if($gameVariables.value(252) == 1){
    valueMenuStandval2 -= 1;
    valueMenuStandval1 = 75;
    valueMenuStandval3 = 0;
  };
...
Scene_Menu.prototype.loadBitmapsMain = function() {
	...
	for (var i = 0; i < $gameParty.members().length; i++) {
		 this._facesBitmaps[i] = ImageManager.loadMenusFaces2("Face_" + $gameParty.members()[i]._actorId);
	};
	this._comBitmaps = []
...
if($gameVariables.value(252) == 5){
  ...
  if($gameParty.members().length < $gameVariables.value(252)){
    return $gameParty.members().length;
  } else {
    return $gameVariables.value(252);
  };
};
};
...
Scene_Menu.prototype.createCharStatus = function() {
	 if($gameSwitches.value(87)){return}
	 this._charStatus = [];
         for (var i = 0; i < $gameParty.battleMembers().length; i++) {	
             this._charStatus[i] = new MCharStatus(i,$gameParty.members()[i],$gameParty.battleMembers().length);   
	 //for (var i = 0; i < this.maxMembers(); i++) {	   
...
Scene_Menu.prototype.createCommands = function() {
	 ...
	 for (var i = 0; i < this._comList.length; i++) {
		 ...
		 this._commands[i].y = 700;
		 this._commands[i].opacity = 255;
		 this._compos[i] = [Moghunter.scMenu_ComX + (90 * i),Moghunter.scMenu_ComY];//変更[Moghunter.scMenu_ComX + (48 * i),Moghunter.scMenu_ComY + (48 * h)]
		 this._comzoom[i] = 0;
...
Scene_Menu.prototype.commandMoveTo = function(value,real_value) {
	...
	var dnspeed = 7 + (Math.abs(value - real_value) / 20);//変更3から7
...
MBustMenu.prototype.posX = function() {
      if($gameVariables.value(252) == 1){
	var space = Math.floor((Graphics.boxWidth + 16) / (1));
      } else {
	var space = Math.floor((Graphics.boxWidth + 16) / (this._maxMembers + 1));
      }
...
MBustMenu.prototype.createCharaters = function() {
	 ...
	 this._orgX  = this._char.x;
       //this._char.scale.x = 0.5; //ステプラグインでActorBをActorAへ
       //this._char.scale.y = 0.5;
	 this._char.x -= 50;
	 this._wait = 7 + 5 * this._index;//変更
...
MCharStatus.prototype.initialize = function(index,actor,maxmembers) {
	...
	this._actor = actor;
if($gameVariables.value(252) == 1){
	this._maxMembers = $gameParty.battleMembers().length;
} else {
	this._maxMembers = maxmembers;
};
	//this._maxMembers = maxmembers;
...
MCharStatus.prototype.loadBitmaps = function() {
   ...
   this._numberImg4 = ImageManager.loadMenusMain("LVNumber");
...
MCharStatus.prototype.posX = function() {
       ...
       if($gameVariables.value(252) == 5){
var space = Math.floor((Graphics.boxWidth - 64) / this._maxMembers + 1);
    return -166 + (space / 2) + (space * this._index);
       } else {
var space = 170
    return -250 + (space / 2.1) + (space * this._index);
       };
...
MCharStatus.prototype.createSprites = function() {
	...
	this.createLVNumber();
	this.createLV2Number();
	this.createTPNumber();
	this.createStates();
        this.createNickName();//変更対か
	this.createName();
};	
...
// * Create NickName
//==============================
MCharStatus.prototype.createNickName = function() {
	this._name = new Sprite(new Bitmap(150,40));;//変更120→150,32→40
	this._name.x = this._layout.x + Moghunter.scMenu_NameX-15;//変更-15
	this._name.y = this._layout.y + Moghunter.scMenu_NameY-10;//変更-10
	this._name.bitmap.fontSize = Moghunter.scMenu_NameFontSize+2;//変更+2
	this._name.bitmap.drawText(this._actor.nickname(),0,0,150,40,"center");
...
MCharStatus.prototype.createName = function() {
	this._name = new Sprite(new Bitmap(150,40));;//変更120→150,32→40
	this._name.x = this._layout.x + Moghunter.scMenu_NameX-15;//変更-15
	this._name.y = this._layout.y + Moghunter.scMenu_NameY-10+175;//変更-10
	this._name.bitmap.fontSize = Moghunter.scMenu_NameFontSize-2;//変更+2
	this._name.bitmap.drawText(this._actor.name(),0,0,150,40,"center");
	this.addChild(this._name);
};
...
MCharStatus.prototype.createStates = function() {
	...
	this._state_icon.x = this._layout.x + Moghunter.scMenu_statesX + 28;//変更+28
...
// * create LV2 Number
//==============================
MCharStatus.prototype.createLV2Number = function() {
    this._lv2_number = [];
	this._NumberData3 = [this._numberImg3.width / 10,this._numberImg3.height]
	var x = this._layout.x + Moghunter.scMenu_LVNumberX+50;
	var y = this._layout.y + Moghunter.scMenu_LVNumberY;
	for (var i = 0; i < 3; i++) {
		 this._lv2_number[i] = new Sprite(this._numberImg3);
		 this._lv2_number[i].visible = false;
		 this._lv2_number[i].opacity = 255;
		 this._lv2_number[i].x = x ;
		 this._lv2_number[i].y = y ;
		 this.addChild(this._lv2_number[i]);		 
	};	
if(this._actor.subclass()){
	this.refresh_number(this._lv2_number,this._actor.classLevel(this._actor._subclassId),this._NumberData3,x)
} else {
	this.refresh_number(this._lv2_number,0,this._NumberData3,x)
};
};	
...
MCharStatus.prototype.createTPNumber = function() {
	...
	var x = this._layout.x + Moghunter.scMenu_MPNumberX;
	var y = this._layout.y + Moghunter.scMenu_MPNumber2Y+20;
	for (var i = 0; i < 3; i++) {
		 this._tp_number[i] = new Sprite(this._numberImg4);
	...
	this.refresh_number(this._tp_number,this._actor.tp,this._NumberData4,x)
};
...
//==============================//変更$gameParty.battleMembers().length;console.log(this._actor.index());
MCharStatus.prototype.createLayoutStatus = function() {
     this._layout = new Sprite(this._layoutImg);
       if($gameVariables.value(252) >= 2 && $gameVariables.value(252) <= 4 && $gameParty.battleMembers().length == 2){
         if(this._actor.index() <= 0){
           this._layout.x = this.posX() - 70 + Moghunter.scMenu_layoutStatusX;
	     this._layout.y = Graphics.boxHeight - 280 + Moghunter.scMenu_layoutStatusY - 220;
         } else {
	     this._layout.x = this.posX() - 70 + Moghunter.scMenu_layoutStatusX - 165;
	     this._layout.y = Graphics.boxHeight - 280 + Moghunter.scMenu_layoutStatusY;
         };
       } else {
         if($gameVariables.value(252) >= 2 && $gameVariables.value(252) <= 4){
           if(this._actor.index() <= 1){
	       this._layout.x = this.posX() - 70 + Moghunter.scMenu_layoutStatusX;
	       this._layout.y = Graphics.boxHeight - 280 + Moghunter.scMenu_layoutStatusY - 220;
           } else {
	       this._layout.x = this.posX() - 70 + Moghunter.scMenu_layoutStatusX - 330;
	       this._layout.y = Graphics.boxHeight - 280 + Moghunter.scMenu_layoutStatusY;
           };
         } else {
	   this._layout.x = this.posX() - 70 + Moghunter.scMenu_layoutStatusX;
