...
Game_System.prototype.canSynthesize = function(item, times) {
    ...
    times = times || 1;
    if (!$gameSwitches.value(206) && item.synthCost * times > $gameParty.gold()) return false;//変更。条件にホームを追加
...
Window_SynthesisList.prototype.windowWidth = function() {
    return Graphics.boxWidth / 2 + 200;
};

Window_SynthesisList_maxCols = Window_SynthesisList.prototype.maxCols; //追加アイテムコアから転記改変
Window_SynthesisList.prototype.maxCols = function() {
    if (SceneManager._scene instanceof Scene_Synthesis) return 2;  //変更1から2
    return Window_SynthesisList_maxCols.call(this);
};
...
Window_SynthesisList.prototype.drawItem = function(index) {
    ...
    this.drawItemName(item, rect.x, rect.y, rect.width - 50); //変更-50
...
Window_SynthesisIngredients.prototype.drawItemDetails = function(index, wy) {
    ...
    var ww = this.contents.width - 50; //変更-50
...
Scene_Synthesis.prototype.createGoldWindow = function() {
    ...
    this._goldWindow.width = Graphics.boxWidth / 2 - 200;
...
Scene_Synthesis.prototype.doBuy = function(number) {
    ...
    if(!$gameSwitches.value(206)){$gameParty.loseGold(price)};//変更。条件追加。ホームでは0
    for (var i = 0; i < this._item.synthIngredients.length; ++i) {
