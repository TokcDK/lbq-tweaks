...
Game_Actor.prototype.learnSkill = function(skillId) {
    ...
    //var hasLearnedSkill2 = this.isLearnedSkillRaw2(skillId);
    ...
    if (!hasLearnedSkill) {
      var value1 = 0;//変更。追加
      if($dataSkills[skillId].stypeId === 10){var value1 = this._battleSkills.length-200};
      if($dataSkills[skillId].stypeId === 15){var value1 = this._battleSkills.length-100};
      var slotId = this._battleSkills.indexOf(0,value1);
...
//Game_Actor.prototype.isLearnedSkillRaw2 = function(skillId) {//変更。追加
//    return this.addedSkills().contains(skillId);
//};
...
Window_SkillEquip.prototype.maxCols = function() { //変更。Sellからの追加処理。Tirアイコンが表示されない
    return 2; //変更1から2
};
...
Scene_Skill.prototype.createSkillEquipWindow = function() {
  ...
  var ww = Graphics.boxWidth / 2+200;//変更。+250
...
Scene_Skill.prototype.openSkillEquipWindows = function() {
    ...
    this._skillEquipWindow.select(0);//0
...
Scene_Skill.prototype.onSkillEqOk = function() {
    ...
    if (skillId != 0) {//変更、処理追加
      if($dataSkills[skillId].meta['Skill Tier']){
        var value3 = $dataSkills[skillId].meta['Skill Tier'].replace(' ','');
        value3 = Number(value3);
        var value26 = 2;
        if(value3 == 6){var value26 = 27};
        TickerManager.show(`${this.actor().name()}が\\C[${value26}]<\x1bI[${Yanfly.Icon.ESTier[value3]}]${Yanfly.Param.ESTierName[value3]}>\\C[0]に\x1bSIN[${skillId}]を装着しました。`);//変更。ティッカー挿入
      };
    };
    this.actor().refresh();
};
...
//Window_SkillList.prototype.selectLast = function() { //変更。スキル装備欄表示の時、カーソルを空欄では無く左上に表示させるもの
//    var skill;
//    if ($gameParty.inBattle()) {
//        skill = this._actor.lastBattleSkill();
//    } else {
